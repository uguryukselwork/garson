...
import { Bell, CheckCircle2, Clock, MapPin, ReceiptText, CreditCard, LayoutGrid, Users, Utensils, Volume2, VolumeX, Banknote, ImagePlus, Settings, X, ChevronRight, MessageSquareQuote, MousePointer2, HelpCircle, Droplets, Receipt, Sparkles, UtensilsCrossed, MessageSquare, Trash2, RefreshCcw, Zap, ShoppingCart, Volume1, Headphones, Radio, Move, Maximize2, Save, Undo2, PlayCircle, Plus, Edit3 } from 'lucide-react';
...
export const StaffDashboard: React.FC<StaffDashboardProps> = ({ 
  ...
  const getTableData = (tableId: string) => {
    ...
    // Calculate total bill: sum of all orders for this table that aren't completed
  ...
  const handleImageUpload = (id: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        updateMenuItemImage(id, base64String);
      };
                ...
                return (
                  <div 
                    ...
                    className={`absolute group rounded-2xl flex flex-col items-center justify-center border-2 transition-all cursor-pointer ${
                      hasCall ? 'bg-red-50 border-red-500 shadow-lg shadow-red-100 animate-pulse' :
                      hasOrder ? 'bg-blue-50 border-blue-400 shadow-sm' :
                      'bg-white border-slate-100 hover:border-slate-300'
                    } ${isSelected ? 'ring-4 ring-orange-500/20 z-20 scale-105' : 'z-10'}`}
                    style={{
                      left: `${layout.x}%`,
                      top: `${layout.y}%`,
                      width: `${layout.w}px`,
                  ...
                  >
                    <span className={`text-[10px] font-black text-center px-1 truncate w-full transition-colors ${isSelected ? 'text-orange-600' : 'text-slate-700'}`}>
                      {layout.name || `T${layout.id}`}
                    </span>
                    
                    {!isEditMode && tableTotal > 0 && (
                      <div className="mt-0.5 px-1 bg-emerald-100 rounded-md border border-emerald-200/50 shadow-sm">
                        <span className="text-[8px] font-black text-emerald-700 whitespace-nowrap">
                          {tableTotal}₺
                        </span>
                      </div>
                    )}
                    ...
                    {isEditMode && (
                      <>
                        ...
                        <div onMouseDown={(e) => onResizeStart(layout.id, e)} className="absolute bottom-1 right-1 cursor-nwse-resize text-orange-400"><Maximize2 size={10} /></div>
                        <div className="absolute top-1 left-1 text-slate-300 pointer-events-none"><Move size={10} /></div>
                      </>
                    )}
                    
                    {!isEditMode && hasCall && <Bell size={12} className="text-red-500 absolute -top-1 -right-1" />}
                  </div>
                );
              })}
            </div>
                          ...
                          <div className="space-y-1.5">
                            {order.items.map((item, idx) => (
                              <div key={idx} className="space-y-1">
                                ...
                                {item.note && (
                                  <div className="flex items-start gap-1 text-[9px] text-slate-400 italic bg-slate-50 p-1 rounded border border-slate-100">
                                    <MessageSquareQuote size={9} className="mt-0.5 shrink-0" />
                                    <span>{item.note}</span>
                                  </div>
                                )}
                              </div>
                      ...
                      <>
                        ...
                        <div className="space-y-4 max-h-[50vh] overflow-y-auto no-scrollbar pr-1">
                          ...
                          <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100">
                            <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">TOPLAM HESAP</p>
                            <p className="text-2xl font-black text-emerald-700">{details.tableTotal}₺</p>
                          </div>

                          {!isEditMode && details.tableCalls.length > 0 && (
                            <div className="space-y-2">
                              <p className="text-[10px] font-black text-red-500 uppercase tracking-widest">Çağrılar</p>
                          ...
                          {!isEditMode && details.tableOrders.length > 0 && (
                            <div className="space-y-2">
                              <p className="text-[10px] font-black text-blue-500 uppercase tracking-widest">Siparişler</p>
